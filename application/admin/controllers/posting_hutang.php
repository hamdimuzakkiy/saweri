...
class posting_hutang extends My_Controller
{
	...
	function index()
	{
		...
		$data['results'] = $this->hutang->getItem_posting();		$data_posting = $this->hutang->getItem_posting();		//$count_data_posting = $this->hutang->count_getItem();		
		//$data['sebagai'] = $sebagai;		// if ($count_data_posting==0){			// $data['PO_NO'][]= null;			// $data['TANGGAL'][]		= null;			// $data['KODE_PARTNER'][]	= null;			// $data['KOUNIT'][]		= null;			// $data['JUMLAH'][]		= null;			//$data['NAMA_KOUNIT'][]	= null;		// }else{
			foreach($data_posting->result() as $rows_posting){										$data['PO_NO'][]		= $rows_posting->po_no;					$data['KODE_PO'][]			= $rows_posting->po_no;					$data['ID'][]			= $rows_posting->id;					$data['TANGGAL'][]		= $rows_posting->TANGGAL;					$data['KODE_PARTNER'][]	= $rows_posting->KODE_PARTNER;					$data['NAMA_PARTNER'][]	= $rows_posting->nama_supplier;					$data['GLID_ANGSURAN'][]= $rows_posting->GLID;					$data['ANGSURAN'][]		= $rows_posting->ANGSURAN;					$data['SISA'][]			= $rows_posting->SISA;					$data['KOUNIT'][]		= $rows_posting->KOUNIT;					$data['NAMA_KOUNIT'][]	= $rows_posting->nama_cabang;					$data['JUMLAH'][]		= $rows_posting->TOTAL;					$data['CARA_BAYAR'][]	= $rows_posting->cara_bayar;			}		//}		
